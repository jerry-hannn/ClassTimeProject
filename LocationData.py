#Database for ClassTime!
#Unfortunately this only contains data from my fall quarter classes
#Checking with Ethan about his JS script to interface with Canvas
#Added new common buildings

ea = {
    "ea1":"Technological%Institute",
    "ea1_dis":"Frances%Searle$Building"
}
cs = {
    "cs 111-0":"Technological%Institute",
    "cs 211-0":"Technological%Institute"
}
math = {
    "math 220-1":"Technological%Institute",
    "math 220-1_dis":"Lunt%Hall"
}
dtc = {
    "dtc lecture":"Ford%Motor$Company$Engineering$Design$Center",
    "dtc dis":"Technological%Institute"
}
Dorms = {
    "636 emerson":"636%Emerson%St%Evanston%Illinois",
    "640 emerson":"640%Emerson%St%Evanston%Illinois",
    "710 emerson":"710%Emerson%St%Evanston%Illinois",
    "720 emerson":"720%Emerson%St%Evanston%Illinois",
    "1838":"1838%Chicago%Ave%Evanston%Illinois",
    "1838 chicago":"1838%Chicago%Ave%Evanston%Illinois",
    "1856 orrington":"1856%Orrington%Ave%Evanston%Illinois",
    "1856 orrington ave":"1856%Orrington%Ave%Evanston%Illinois",
    "smq":"South%Mid%Quads"
}
other = {
    "spac":"Henry%Crown%Sports%Pavilion",
    "mudd":"Mudd%Library",
    "blom":"Blomquist%Recreation%Center",
    "main":"1970%Campus%Dr$Evanston$Illinois",
    "norris":"Norris%University%Center",
    "kellogg":"Kellogg%Global%Hub",
    "bienen":"Bienen%School%Of%Music",
    "wirtz":"The%Virginia%Wadsworth%Wirtz%Center",
    "tech":"Northwestern$University$Technological$Institute",
    "deering":"Deering$Library",
    "Regenstein":"Regenstein$Hall$Of$Music",
}
